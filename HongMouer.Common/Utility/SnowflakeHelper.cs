...
namespace HongMouer.Common.Utility
{
    ...
    {
        private static long machineId= 2L;//机器ID  
        private static long datacenterId = 2L;//数据ID  
        private static long sequence = 0L;//计数从零开始  

        //private static long twepoch = 687888001020L; //唯一时间随机量  

        private static long twepoch = 14200416L;//2015-01-01年开始的时间戳
        ...
        public static long NextId()
        {
            lock (ConstLock.syncRoot)
            {
                ...
                if (timestamp < lastTimestamp)
                {
                    throw new HongMouerException("时间戳比上一次生成ID时时间戳还小，故异常");
                }
